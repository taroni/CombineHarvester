...
for infile in onlyfiles:
    ...
    cb.cp().process(["LFV"]).AddSyst(cb,"TheoH", "lnN", ch.SystMap('mass')(["200","300"], 1.018)(["450"],1.02)(["600","750"], 1.021)(["900"],1.022) )
    cb.cp().process(["LFV"]).AddSyst(cb,"TheoHPDF", "lnN", ch.SystMap('mass')(["200","300"], 1.03)(["450"],1.031)(["600"], 1.035)(["750"], 1.04)(["900"],1.046) )
   ...
   # print cb.cp().backgrounds()
    ...
    mH=ROOT.RooRealVar("mH", "mH", 200, 900)
    ws=ROOT.RooWorkspace ("lfv", "lfv");

    massRange = 'LowMass'
    if 'HighMass' in infile: massRange='HighMass'

    ##print  "Scaling signal process rates for x-sec..."
    ##xs = { }
    ##for p in sig_procs:
    ##    # Get the table of xsecs vs mass for process 'p' and era 'e':
    ##    xs[p] = ch.TGraphFromTable("x_sec_accept.txt", "mH", "xsec");
    ##    print '>>>> Scaling for process ' + p
    ##    cb.cp().process([p]).ForEachProc(
    ##        lambda x : x.set_rate(
    ##            x.rate() * xs[p].Eval(float(x.mass())) * xs[p].Eval(float(x.mass())))
    ##    )
    ##
    ##    print "done"
    ...
    demo=ROOT.TFile("et_demo_%s.root" %massRange, "RECREATE")
    do_morphing = True
    mass_var = {
      "LFV" : mH
    }
    if (do_morphing) :
        ...
        print bins
        for  b in bins :
            procs = cb.cp().bin([b]).signals().process_set()
            print procs
            for p in procs:
                print p
                morphing.BuildRooMorphing(ws, cb, b, p, mass_var[p], "norm", True, True, False, demo)
    ...
    demo.Close();
    #cb.AddWorkspace(ws);
    #cb.cp().signals().ExtractPdfs(cb, "lfv", "$BIN_$PROCESS_morph");
    #cb.PrintAll();
    ...
    for b in  bins:
       for  m in masses:
           output=ROOT.TFile(dirname+"/%s_%s_morf.input.root"%(b,m), "RECREATE");
    
           print ">> Writing datacard for bin: ",  b , " and mass: " , m
           cb.cp().bin([b]).mass([m, "*"]).WriteDatacard(dirname+'/'+ b + "_" + m + "_morf.txt", output.GetName())
