...
#onlyfiles = ['shapes_em_LowMass.root']
onlyfiles = ['shapes_em_HighMass.root', 'shapes_em_LowMass.root']
...
for infile in onlyfiles:
    ...
    bkg_procs = [ "DY", "DYTT", "ttbar", "singlet","EWKDiboson", "QCD", "W" ] # I removed SMH
    mcbkg_procs = [ "DY", "DYTT", "ttbar", "singlet","EWKDiboson", "W"] # I removed SMH
    cb.AddProcesses(["*"], ["lfv"], ["13TeV"], ["em"], bkg_procs, cats, signal=False)

    sig_procs = ["LFV"]
    cb.AddProcesses(masses, ["lfv"], ["13TeV"], ["em"], sig_procs, cats, signal=True)

    cb.cp().process(sig_procs+mcbkg_procs).AddSyst(cb, "CMS_eff_e", "lnN", ch.SystMap()(1.02))# I removed SMH

    cb.cp().process(sig_procs+mcbkg_procs).AddSyst(cb, "CMS_eff_mu", "lnN", ch.SystMap()(1.02))# I removed SMH
    ...
    cb.cp().process(["SMH","DYTT","EWKDiboson"]).AddSyst(cb, "highPtTau", "shape", ch.SystMap()(1.0))
