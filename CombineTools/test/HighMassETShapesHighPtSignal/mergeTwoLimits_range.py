...
parser.add_argument('-d','--dir', dest='MassDir', type=str, nargs='+', help='jet directory', required=True)
args = parser.parse_args()
jet = args.jetDir[0]
print args.MassDir
mydir0 =args.MassDir[0]
mydir1 =args.MassDir[1]
...
ModTDRStyle()
canv = ROOT.TCanvas('limit%sBrXsec_LowHighMasses'%jet, 'limit%sBrXsec_2_LowHighMasses'%jet)
pads = OnePad()
...
axis.GetYaxis().SetRangeUser(0.0005, 2.)
...
legend_dict = {
         ...
         'exp0' : { 'Label' : 'Expected', 'LegendStyle' : 'LP', 'DrawStyle' : 'LSAME'},
...
mygraph=graphs['exp0'].Clone()
mygraph.RemovePoint(mygraph.GetN()-1)
mygraph.Draw("PSAME")
...
graphs1['exp0'] = graphFromJson(js, 'exp0')

graphs1['exp1'] = graphBandFromJson(js, 'exp0', 'exp-1', 'exp+1')

graphs1['exp2'] = graphBandFromJson(js, 'exp0', 'exp-2', 'exp+2')
