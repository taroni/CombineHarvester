...
for infile in onlyfiles:
   ...
   # print cb.cp().backgrounds()
    ...
    ws=ROOT.RooWorkspace ("het", "het");

    massList = ch.MassesFromRange("200-900:25");
    
    demo=ROOT.TFile("et_demo.root", "RECREATE")
    do_morphing = True;
    mass_var = {
      "ggH_Hhhbbtautau": mH
    }
    if (do_morphing) :
        ...
        for  b in bins :
            ...
            for p in procs:
                morfing.BuildRooMorphing(ws, cb, b, p, mass_var[p], "norm", True, True, True, demo)
        
      
    
    demo.Close();
    cb.AddWorkspace(ws);
    cb.cp().signals().ExtractPdfs(cb, "het", "$BIN_$PROCESS_morph");
    cb.PrintAll();
