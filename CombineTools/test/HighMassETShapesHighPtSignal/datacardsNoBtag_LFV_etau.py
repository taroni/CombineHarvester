...
onlyfiles = ['shapes_HighMass_noBveto.root', 'shapes_LowMass_noBveto.root']
...
for infile in onlyfiles:
    ...
    #cb.cp().process(sig_procs+["ttbar", "singlet"]).AddSyst(cb, "btagVeto", "lnN", ch.SystMap()(1.05))
    
    cb.cp().process(["SMH","LFV"]).AddSyst(cb,"TheoH", "lnN", ch.SystMap()(1.10))

    cb.cp().process(["fakes"]).AddSyst(cb, "norm_taufakes_etauhad", "lnN", ch.SystMap()(1.20))
